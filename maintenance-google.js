...
async function searchOne(p){
  await ensureGoogle();const {Place}=await google.maps.importLibrary('places');const q=[p['名称'],p['住所'],'下諏訪町 長野県'].filter(Boolean).join(' ');const {places}=await Place.searchByText({textQuery:q,fields:['id','displayName','formattedAddress','location','googleMapsURI','websiteURI'],language:'ja',region:'jp',maxResultCount:5});if(!places?.length)return null;const ranked=places.map(x=>({x,s:score(p,x)})).sort((a,b)=>b.s-a.s);const best=ranked[0];if(best.s<65||!String(best.x.formattedAddress||'').includes('下諏訪町'))return null;return best.x;
}
function saveGoogle(p,x){const lat=x.location?.lat?.(),lng=x.location?.lng?.();if(!Number.isFinite(lat)||!Number.isFinite(lng))return false;p.latitude=lat;p.longitude=lng;p['座標ステータス']='Google Places再取得';p['座標情報源']='Google Places API';p.google_place_id=x.id||'';p['Google確認住所']=x.formattedAddress||'';p['GoogleマップURL_確定']=x.googleMapsURI||p['Googleマップ検索URL']||'';if(x.websiteURI)p['公式WebページURL']=x.websiteURI;try{localStorage.setItem('gmap-pin:'+p.place_id,JSON.stringify({lat,lng,url:p['GoogleマップURL_確定'],googlePlaceId:p.google_place_id,formattedAddress:p['Google確認住所'],status:'Google Places再取得'}))}catch(e){}const b=BASE.find(v=>PlaceData.keyOf(v)===PlaceData.keyOf(p));if(b)Object.assign(b,p);return true}

...
extra_file = root / 'data' / 'places_toilets_user.csv'

with services.open(encoding='utf-8-sig', newline='') as f:
    base = list(csv.DictReader(f))
with extra_file.open(encoding='utf-8-sig', newline='') as f:
...
print(f'merged {len(extra)} user-confirmed toilet places into services; total {len(order)}')
